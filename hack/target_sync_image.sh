...
main(){
    ...
    skopeo sync --multi-arch all "$SRC_TLS_VERIFY" "$DEST_TLS_VERIFY" -s docker -d docker --dest-username "$DOCKER_USER" --dest-password "$DOCKER_PASSWORD" "$ORIGIN_IMAGE" "$DOCKER_REGISTRY"
    if [ $? -ne 0 ]; then
